                ...
                TextParserV1.registerDefault(
                        TextParserV1.TextTag.of(
                                ...
                                (tag, data, input, handlers, endAt) -> {
                                    ...
                                    return new TextParserV1.TagNodeValue(new ChatFormattingNode(out.nodes(), formatting), out.length());
                                })
                );
            }
        }

        {
            TextParserV1.registerDefault(
                    TextParserV1.TextTag.of(
                            ...
                            (tag, data, input, handlers, endAt) -> {
                                var out = recursiveParsing(input, handlers, endAt);
                                return new TextParserV1.TagNodeValue(new ColorNode(out.nodes(), TextColor.parseColor(cleanArgument(data))), out.length());
                            }
                    )
            );
        }
        {
            TextParserV1.registerDefault(
                    TextParserV1.TextTag.of(
                            ...
                            (tag, data, input, handlers, endAt) -> {
                                var out = recursiveParsing(input, handlers, endAt);
                                return out.value(new FontNode(out.nodes(), ResourceLocation.tryParse(cleanArgument(data))));
                            }
                    )
            );
        }
                    ...
                    TextParserV1.TextTag.of(
                            ...
                            (tag, data, input, handlers, endAt) -> {
                                ...
                                if (textColors.size() == 0) {
                                    textColors.add(TextColor.fromLegacyFormat(ChatFormatting.WHITE));
                                    textColors.add(TextColor.fromLegacyFormat(ChatFormatting.WHITE));
                                } else if (textColors.size() == 1) {
                                    textColors.add(textColors.get(0));
                                }

                                GeneralUtils.HSV hsv = GeneralUtils.rgbToHsv(textColors.get(0).getValue());
                                ...
                                return out.value(new GradientNode(out.nodes(), (pos, length) -> {
                                    ...
                                    GeneralUtils.HSV colorA = GeneralUtils.rgbToHsv(textColors.get(Math.min((int) (pos / sectionSize), colorSize - 1)).getValue());
                                    GeneralUtils.HSV colorB = GeneralUtils.rgbToHsv(textColors.get(Math.min((int) (pos / sectionSize) + 1, colorSize - 1)).getValue());
