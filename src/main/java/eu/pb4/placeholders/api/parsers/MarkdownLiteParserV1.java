...
import eu.pb4.placeholders.api.node.parent.ChatFormattingNode;
...
public final class MarkdownLiteParserV1 implements NodeParser {
    ...
    public static TextNode defaultSpoilerFormatting(TextNode[] textNodes) {
        return new StyledNode(new TextNode[]{TextNode.of("["), new TranslatedNode("options.hidden"), TextNode.of("]")},
                Style.EMPTY.withColor(ChatFormatting.GRAY).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.empty())).withItalic(true),
                new ParentNode(textNodes), null, null);

    }

    public static TextNode defaultQuoteFormatting(TextNode[] textNodes) {
        return new StyledNode(textNodes, Style.EMPTY.withColor(ChatFormatting.GRAY).withItalic(true), null, null, null);
    }
        ...
        while (nodes.hasNext()) {
            ...
            if (next.type == SubNodeType.TEXT_NODE) {
                ...
                if (value != null) {
                    ...
                    out.add(new ChatFormattingNode(value, ChatFormatting.STRIKETHROUGH));
                    continue;
                }
                    ...
                    if ((next.type == SubNodeType.STAR && this.allowedFormatting.contains(MarkdownFormat.BOLD))
                        ...
                        if (nexter.type == next.type) {
                            ...
                            if (value != null) {
                                ...
                                out.add(new ChatFormattingNode(value, next.type == SubNodeType.STAR ? ChatFormatting.BOLD : ChatFormatting.UNDERLINE));
                                continue;
                            }
                        }
                        nodes.previous();
                ...
                if (!two && this.allowedFormatting.contains(MarkdownFormat.ITALIC)) {
                    ...
                    if (value != null) {
                        ...
                        out.add(new ChatFormattingNode(value, ChatFormatting.ITALIC));
                        continue;
                    }
                }
            }
